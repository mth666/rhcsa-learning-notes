...
To make it a script so that the system can run it, chmod must grant execution rights : `chmod +x /path/to/scripts/script.sh`

Exit status codes (exit)

Every single command or script run in Linux leaves behind an invisible report card called an Exit Status (an integer between 0 and 255).

- 0 means Success, Everything completed perfectly.
- any number from 1 to 255 means failure or an error has occurred.
In a script, the script will automatically return the exit status of the very last command that ran inside it. explicitly declaring exit 0 at the very bottom when a script fulfills its duties cleanly would be a good habbit.
And everything else - variables, loops, conditionals are just building on top of the above three things.